const BYTEWATT_REPORT_CARD_BUILD = "001";
...
class ByteWattReportCard extends HTMLElement {
  ...
  _fmtPower(value) {
    const number = Number(value);
    if (!Number.isFinite(number)) return "Unavailable";
    return `${this._fmtNumber(number, 0)} W`;
  }
  ...
  _renderChart(reporting) {
    ...
    const tickLabels = xTicks.map((point) => times[Math.min(times.length - 1, Math.max(0, Math.round(point * (times.length - 1))))] || "");
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .selector-row { display:grid; grid-template-columns: 160px minmax(0, 1fr); gap:16px; align-items:center; }
        .label { font-size:0.95rem; font-weight:700; color:#31435d; }
        select {
          width:min(360px, 100%); padding:12px 14px; border-radius:14px; border:1px solid rgba(51, 92, 140, 0.18);
          background:#fff; color:#17263a; font-size:0.95rem;
        }
        .summary-grid, .live-grid, .stats-grid { display:grid; gap:14px; }
        .summary-grid { grid-template-columns: repeat(5, minmax(0, 1fr)); }
        .live-grid { grid-template-columns: repeat(5, minmax(0, 1fr)); }
        .stats-grid { grid-template-columns: repeat(4, minmax(0, 1fr)); }
        .summary-tile, .live-tile, .stat-card, .aggregate-card {
          background:#fff; border-radius:18px; border:1px solid rgba(51, 92, 140, 0.12); padding:16px 18px;
          box-shadow: 0 10px 20px rgba(20, 44, 78, 0.06);
        }
        .summary-label, .live-label, .stat-label, .aggregate-title {
          font-size:0.82rem; font-weight:700; letter-spacing:0.04em; text-transform:uppercase; color:#597291;
        }
        .summary-value, .live-value, .stat-value { margin-top:8px; font-size:1.45rem; font-weight:800; color:#14243a; }
        .aggregate-strip { display:grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap:12px; }
        .aggregate-metric { margin-top:6px; color:#31435d; font-size:0.9rem; }
        .body-grid { display:grid; grid-template-columns: 1.15fr 1fr; gap:18px; align-items:start; }
        .panel {
          background:#fff; border-radius:22px; border:1px solid rgba(51, 92, 140, 0.12); padding:18px;
        ...
        .panel-title { font-size:1.1rem; font-weight:800; color:#17263a; }
        .panel-date {
          padding:8px 12px; border-radius:999px; background:#eef5ff; color:#36567b; font-size:0.9rem; font-weight:700;
        }
        .energy-layout {
          display:grid; grid-template-columns: 1fr 1fr 1fr; gap:16px; align-items:center;
        }
        .energy-node {
          background:#f8fbff; border:1px solid rgba(51, 92, 140, 0.1); border-radius:18px; padding:16px; text-align:center;
        }
        .energy-node.battery { grid-column:2; grid-row:1 / span 2; align-self:stretch; display:grid; align-content:center; gap:8px; }
        .energy-node-title { font-size:0.92rem; font-weight:800; color:#355377; text-transform:uppercase; letter-spacing:0.05em; }
        .energy-node-value { font-size:1.5rem; font-weight:800; color:#15253d; }
        .energy-node-sub, .energy-bridge { font-size:0.88rem; color:#4d6787; }
        .energy-bridge { text-align:center; font-weight:700; }
        .chart-header { margin-bottom:18px; }
        .panel-tabs { display:flex; gap:10px; }
        .panel-tabs button, .legend-chip {
          border:none; border-radius:999px; padding:8px 14px; font-weight:700; cursor:pointer;
        }
        .panel-tabs button { background:#e8eff8; color:#476687; }
        .panel-tabs button.active { background:#2f75d8; color:#fff; }
        .ring-grid { display:grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap:14px; margin-bottom:18px; }
        .ring-card {
          border-radius:20px; padding:16px 14px; text-align:center; border:2px solid transparent; background:#fbfdff;
        }
        ...
        .ring-value { font-size:1.1rem; font-weight:800; color:#17263a; }
        .ring-label { margin-top:8px; color:#506884; font-size:0.88rem; }
        .chart { width:100%; height:auto; display:block; background:#fff; border-radius:18px; }
        .axis, .grid { stroke:#d8e3ef; stroke-width:1; }
        .tick, .axis-label { fill:#6280a2; font-size:12px; font-weight:700; }
        .legend-row {
          display:flex; flex-wrap:wrap; gap:10px; margin-top:14px;
        }
        .legend-chip { background:#eef4fb; color:#4a6787; }
        .legend-chip.active { background:#1f2e43; color:#fff; }
        .empty {
          padding:24px; border-radius:18px; background:#fff; border:1px dashed rgba(51, 92, 140, 0.24); color:#496681;
        }
        @media (max-width: 1180px) {
          .summary-grid, .live-grid { grid-template-columns: repeat(3, minmax(0, 1fr)); }
          .ring-grid { grid-template-columns: repeat(3, minmax(0, 1fr)); }
          .body-grid { grid-template-columns: 1fr; }
        }
        @media (max-width: 780px) {
          .selector-row { grid-template-columns: 1fr; }
          .summary-grid, .live-grid, .stats-grid, .ring-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
          .energy-layout { grid-template-columns: 1fr; }
          .energy-node.battery { grid-column:auto; grid-row:auto; }
        }
      </style>
      <ha-card>
        <div class="shell">
