const BYTEWATT_POLICY_CARD_BUILD = "011";
...
class ByteWattPolicyCard extends HTMLElement {
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .charge-footer-actions {
          gap: 8px;
        }
        .charge-footer-actions button {
          height: 34px;
          font-size: 0.9rem;
        }
        .schedule-empty {
  ...
  _readSummaryNumber(primaryEntityId, suffixes = [], friendlyNames = []) {
    ...
    const suffixMatch = states.find(([entityId, stateObj]) => {
      if (!entityId.startsWith("sensor.") || !entityId.includes("bytewatt")) return false;
      if (!suffixes.some((suffix) => entityId.endsWith(suffix))) return false;
    ...
    const friendlyMatch = states.find(([, stateObj]) => {
      const friendlyName = String(stateObj?.attributes?.friendly_name || "");
      if (!friendlyNames.includes(friendlyName)) return false;
      return Number.isFinite(Number(stateObj?.state));
    });
    if (friendlyMatch) {
